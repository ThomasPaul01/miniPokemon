...
export const teachAttackToPokemon = async (
  pokemonId: number,
  attackId: number,
  limitUse: number,
) => {
...
export const healPokemon = async (pokemonId: number) => {
  ...
  try {
    ...
    await client.query(`UPDATE pokemons SET life_points = 100 WHERE id = $1`, [pokemonId]);
    ...
    const result = await client.query('SELECT * FROM pokemons WHERE id = $1', [pokemonId]);
    return result.rows[0];
  } catch (error) {
...
export const attackPokemon = async (attackerId: number, defenderId: number) => {
  ...
  try {
    ...
    if (attackResult.rows.length === 0) {
      throw new Error(
        `No available attacks (remaining_uses = 0) for pokemon with id ${attackerId}`,
      );
    }
