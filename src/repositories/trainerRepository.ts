...
export const addPokemonToTrainer = async (trainerId: number, pokemonId: number) => {
  ...
  try {
    ...
    const trainerCheck = await client.query('SELECT id FROM trainers WHERE id = $1', [trainerId]);
    if (trainerCheck.rows.length === 0) {
      throw new Error(`Trainer with id ${trainerId} not found`);
    }

    // Vérifier que le pokémon existe
    const pokemonCheck = await client.query('SELECT id FROM pokemons WHERE id = $1', [pokemonId]);
